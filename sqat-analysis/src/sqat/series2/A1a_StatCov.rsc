...
- how do your results compare to the jpacman results in the paper? Has jpacman improved?
	>Jpacman in the paper has 88.06% for static and 93.53% for clover (in paper). Our result is 66.6%, so our coverage is way worse.

- use a third-party coverage tool (e.g. Clover) to compare your results to (explain differences)
	>clover shows a coverage of 75%. This might be because 4 tests are skipped. Thus not a higher coverage.
	And the difference with out implementation is probably because some methods might indirectly test other methods or because of method declarations in interfaces and such. Which we think are a method that should be tested, but clover doesnt think that. 

*/


int countMethodsInFile(Declaration decls) {
  int methods = 0;
  for(decl <- decls){
  	visit(decl){
...
public int countMethodsInProject(list[loc] locations, str pattern){
	int totalMethods = 0;
	s = 0;
	lst = locations;
	while(!isEmpty(lst)){
...
public void solve1(){		
	...
	println("size of testMethodInvocations: <sizeTestmethodInvocations>");
	sizeNonTestMethods = size(dup(nonTestMethods));
	println("size of nonTestMethods: <sizeNonTestMethods>");
	isCalledByTestMethodInvocationSize = size(dup(isCalledByTest));
	println("isCalledByTestMethodInvocationSize: <isCalledByTestMethodInvocationSize>");
	...
	print("Methods that are not tesed: "); println(nonTestMethods - isCalledByTest);
}
